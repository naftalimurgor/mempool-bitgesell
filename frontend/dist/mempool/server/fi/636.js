    ...
    static #_2 = this.ɵcmp = /*@__PURE__*/_angular_core__WEBPACK_IMPORTED_MODULE_9__["ɵɵdefineComponent"]({
      ...
      consts: () => {
        ...
        if (typeof ngI18nClosureMode !== "undefined" && ngI18nClosureMode) {
          ...
          i18n_0 = "" + "[\uFFFD#1\uFFFD|\uFFFD#3\uFFFD]" + "" + "\uFFFD#2\uFFFD" + "mempool.space tarjoaa vain tietoja Bitcoin-verkosta. " + "\uFFFD/#2\uFFFD" + "Se ei voi auttaa sinua varojen palauttamisessa, lompakko-ongelmissa jne." + "[\uFFFD/#1\uFFFD|\uFFFD/#3\uFFFD]" + "" + "[\uFFFD#1\uFFFD|\uFFFD#3\uFFFD]" + " T\xE4llaisissa pyynn\xF6iss\xE4 sinun on otettava yhteytt\xE4 tahoon, joka auttoi suorittamaan tapahtuman (lompakkosovellus, vaihtoyhti\xF6 jne)." + "[\uFFFD/#1\uFFFD|\uFFFD/#3\uFFFD]" + "";
        }
...
let DocsComponent = /*#__PURE__*/(() => {
  class DocsComponent {
    ...
    ngDoCheck() {
      ...
      if (url[0].path === "faq") {
        this.activeTab = 0;
        this.seoService.setTitle("UKK");
        this.seoService.setDescription("Hanki vastauksia yleisiin kysymyksiin, kuten: Mik\xE4 on mempool? Miksi siirtoni ei vahvistu? Miten voin ajaa omaa The Mempool Open Source Project -instanssia? Ja paljon muuta.");
        this.ogService.setManualOgImage('faq.jpg');
      } else if (url[1].path === "rest") {
        this.activeTab = 1;
        this.seoService.setTitle("REST API");
        if (this.stateService.network === 'liquid' || this.stateService.network === 'liquidtestnet') {
          this.seoService.setDescription("Dokumentaatio liquid.network REST API-palvelulle: hae tietoja osoitteista, siirtotapahtumista, omaisuuserist\xE4, lohkoista ja muusta.");
        } else {
          this.seoService.setDescription("Dokumentaatio mempool.space REST API-palvelulle: hae tietoja osoitteista, siirtotapahtumista, lohkoista, maksuista, louhinnasta, Salamaverkosta ja paljon muuta.");
        }
      } else if (url[1].path === "websocket") {
        this.activeTab = 2;
        this.seoService.setTitle("WebSocket API");
        if (this.stateService.network === 'liquid' || this.stateService.network === 'liquidtestnet') {
          this.seoService.setDescription("Dokumentaatio liquid.networkin WebSocket API-palvelulle: hanki reaaliaikaista tietoa lohkoista, mempoolista, siirtotapahtumista, osoitteista ja paljon muuta.");
        } else {
          this.seoService.setDescription("Dokumentaatio mempool.space WebSocket API-palvelulle: hanki reaaliaikaista tietoa lohkoista, mempooleista, siirtotapahtumista, osoitteista ja muusta.");
        }
      } else {
        this.activeTab = 3;
        this.seoService.setTitle("Electrum RPC");
        this.seoService.setDescription("Dokumentaatio Electrum RPC-liittym\xE4st\xE4mme: saat v\xE4litt\xF6m\xE4n, k\xE4tev\xE4n ja luotettavan p\xE4\xE4syn Esplora-instanssiin.");
      }
    }
    ngOnDestroy() {
