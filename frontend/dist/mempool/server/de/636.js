    ...
    static #_2 = this.ɵcmp = /*@__PURE__*/_angular_core__WEBPACK_IMPORTED_MODULE_9__["ɵɵdefineComponent"]({
      ...
      consts: () => {
        ...
        if (typeof ngI18nClosureMode !== "undefined" && ngI18nClosureMode) {
          ...
          i18n_0 = "" + "[\uFFFD#1\uFFFD|\uFFFD#3\uFFFD]" + "" + "\uFFFD#2\uFFFD" + "mempool.space stellt lediglich Daten \xFCber das Bitcoin-Netzwerk bereit." + "\uFFFD/#2\uFFFD" + " Es kann Ihnen beim Abrufen von Geldern, bei Problemen mit der Wallet usw. nicht helfen." + "[\uFFFD/#1\uFFFD|\uFFFD/#3\uFFFD]" + "" + "[\uFFFD#1\uFFFD|\uFFFD#3\uFFFD]" + "Bei solchen Anfragen m\xFCssen Sie Kontakt mit der Entit\xE4t aufnehmen, die bei der Transaktion geholfen hat (Wallet-Software, B\xF6rsenunternehmen usw.)." + "[\uFFFD/#1\uFFFD|\uFFFD/#3\uFFFD]" + "";
        }
  ...
  class DocsComponent {
    ...
    ngDoCheck() {
      ...
      if (url[0].path === "faq") {
        ...
        this.seoService.setDescription("Erhalten Sie Antworten auf h\xE4ufige Fragen wie: Was ist ein Mempool? Warum wird meine Transaktion nicht best\xE4tigt? Wie kann ich meine eigene Instanz des Mempool Open Source Project ausf\xFChren? Und mehr.");
        this.ogService.setManualOgImage('faq.jpg');
      } else if (url[1].path === "rest") {
        this.activeTab = 1;
        this.seoService.setTitle("REST API");
        if (this.stateService.network === 'liquid' || this.stateService.network === 'liquidtestnet') {
          this.seoService.setDescription("Dokumentation f\xFCr den liquid.network REST API-Dienst: Erhalte Informationen zu Adressen, Transaktionen, Assets, Bl\xF6cken und mehr.");
        } else {
          this.seoService.setDescription("Dokumentation f\xFCr den REST-API-Dienst mempool.space: Erhalte Informationen zu Adressen, Transaktionen, Bl\xF6cken, Geb\xFChren, Mining, dem Lightning-Netzwerk und mehr.");
        }
      } else if (url[1].path === "websocket") {
        this.activeTab = 2;
        this.seoService.setTitle("WebSocket API");
        if (this.stateService.network === 'liquid' || this.stateService.network === 'liquidtestnet') {
          this.seoService.setDescription("Dokumentation f\xFCr den liquid.network WebSocket API-Dienst: Erhalte Echtzeitinformationen zu Bl\xF6cken, Mempools, Transaktionen, Adressen und mehr.");
        } else {
          this.seoService.setDescription("Dokumentation f\xFCr den WebSocket-API-Dienst mempool.space: Erhalte Echtzeitinformationen zu Bl\xF6cken, Mempools, Transaktionen, Adressen und mehr.");
        }
      } else {
        this.activeTab = 3;
        this.seoService.setTitle("Electrum RPC");
        this.seoService.setDescription("Dokumentation f\xFCr unsere Electrum RPC-Schnittstelle: Erhalte sofortigen, bequemen und zuverl\xE4ssigen Zugriff auf eine Esplora-Instanz.");
      }
    }
    ngOnDestroy() {
