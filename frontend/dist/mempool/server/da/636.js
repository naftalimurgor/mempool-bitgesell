    ...
    static #_2 = this.ɵcmp = /*@__PURE__*/_angular_core__WEBPACK_IMPORTED_MODULE_9__["ɵɵdefineComponent"]({
      ...
      consts: () => {
        ...
        if (typeof ngI18nClosureMode !== "undefined" && ngI18nClosureMode) {
          ...
          i18n_0 = "" + "[\uFFFD#1\uFFFD|\uFFFD#3\uFFFD]" + "" + "\uFFFD#2\uFFFD" + "mempool.space leverer blot data om Bitcoin-netv\xE6rket." + "\uFFFD/#2\uFFFD" + " Den kan ikke hj\xE6lpe dig med indhentning af penge, wallet-problemer osv." + "[\uFFFD/#1\uFFFD|\uFFFD/#3\uFFFD]" + "" + "[\uFFFD#1\uFFFD|\uFFFD#3\uFFFD]" + "For s\xE5danne anmodninger skal du kontakte enheden, der hjalp med at foretage transaktionen (wallet software, kryptob\xF8rs osv.)." + "[\uFFFD/#1\uFFFD|\uFFFD/#3\uFFFD]" + "";
        }
  ...
  class DocsComponent {
    ...
    ngDoCheck() {
      ...
      if (url[0].path === "faq") {
        ...
        this.seoService.setDescription("F\xE5 svar p\xE5 almindelige sp\xF8rgsm\xE5l som: Hvad er en mempool? Hvorfor bekr\xE6ftes min transaktion ikke? Hvordan kan jeg k\xF8re min egen instans af Mempool Open Source Project? Og mere.");
        this.ogService.setManualOgImage('faq.jpg');
      } else if (url[1].path === "rest") {
        this.activeTab = 1;
        this.seoService.setTitle("REST API");
        if (this.stateService.network === 'liquid' || this.stateService.network === 'liquidtestnet') {
          this.seoService.setDescription("Dokumentation til liquid.network REST API-tjenesten: f\xE5 info om adresser, transaktioner, aktiver, blokkr og mere.");
        } else {
          this.seoService.setDescription("Dokumentation for mempool.space REST API-tjenesten: f\xE5 info om adresser, transaktioner, blokke, gebyrer, minedrift, Lightning-netv\xE6rket og mere.");
        }
      } else if (url[1].path === "websocket") {
        this.activeTab = 2;
        this.seoService.setTitle("WebSocket API");
        if (this.stateService.network === 'liquid' || this.stateService.network === 'liquidtestnet') {
          this.seoService.setDescription("Dokumentation til liquid.network WebSocket API-tjenesten: f\xE5 realtidsoplysninger om blokke, mempools, transaktioner, adresser og mere.");
        } else {
          this.seoService.setDescription("Dokumentation til mempool.space WebSocket API-tjenesten: f\xE5 realtidsoplysninger om blokke, mempools, transaktioner, adresser og mere.");
        }
      } else {
        this.activeTab = 3;
        this.seoService.setTitle("Electrum RPC");
        this.seoService.setDescription("Dokumentation til vores Electrum RPC-gr\xE6nseflade: f\xE5 \xF8jeblikkelig, bekvem og p\xE5lidelig adgang til en Esplora-instans.");
      }
    }
    ngOnDestroy() {
