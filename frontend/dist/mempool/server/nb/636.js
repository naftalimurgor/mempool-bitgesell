    ...
    static #_2 = this.ɵcmp = /*@__PURE__*/_angular_core__WEBPACK_IMPORTED_MODULE_9__["ɵɵdefineComponent"]({
      ...
      consts: () => {
        ...
        if (typeof ngI18nClosureMode !== "undefined" && ngI18nClosureMode) {
          ...
          i18n_0 = "" + "[\uFFFD#1\uFFFD|\uFFFD#3\uFFFD]" + "" + "\uFFFD#2\uFFFD" + "mempool.space gir bare data om Bitcoin-nettverket." + "\uFFFD/#2\uFFFD" + " Det kan ikke hjelpe deg med \xE5 hente midler, lommebokproblemer osv." + "[\uFFFD/#1\uFFFD|\uFFFD/#3\uFFFD]" + "" + "[\uFFFD#1\uFFFD|\uFFFD#3\uFFFD]" + "For slike foresp\xF8rsler m\xE5 du ta kontakt med enheten som bidro til transaksjonen (lommebokprogramvare, b\xF8rsselskap osv.)." + "[\uFFFD/#1\uFFFD|\uFFFD/#3\uFFFD]" + "";
        }
  ...
  class DocsComponent {
    ...
    ngDoCheck() {
      ...
      if (url[0].path === "faq") {
        ...
        this.seoService.setDescription("F\xE5 svar p\xE5 vanlige sp\xF8rsm\xE5l som: Hva er en mempool? Hvorfor bekreftes ikke transaksjonen min? Hvordan kan jeg kj\xF8re min egen instans av Mempool Open Source Project? Og mer.");
        this.ogService.setManualOgImage('faq.jpg');
      } else if (url[1].path === "rest") {
        this.activeTab = 1;
        this.seoService.setTitle("REST API");
        if (this.stateService.network === 'liquid' || this.stateService.network === 'liquidtestnet') {
          this.seoService.setDescription("Dokumentasjon for liquid.network REST API-tjenesten: f\xE5 informasjon om adresser, transaksjoner, eiendeler, blokker og mer.");
        } else {
          this.seoService.setDescription("Dokumentasjon for mempool.space REST API-tjenesten: f\xE5 informasjon om adresser, transaksjoner, blokker, avgifter, utvinning, Lightning-nettverket og mer.");
        }
      } else if (url[1].path === "websocket") {
        this.activeTab = 2;
        this.seoService.setTitle("WebSocket API");
        if (this.stateService.network === 'liquid' || this.stateService.network === 'liquidtestnet') {
          this.seoService.setDescription("Dokumentasjon for liquid.network WebSocket API-tjenesten: f\xE5 sanntidsinformasjon om blokker, mempooler, transaksjoner, adresser og mer.");
        } else {
          this.seoService.setDescription("Dokumentasjon for mempool.space WebSocket API-tjenesten: f\xE5 sanntidsinformasjon om blokker, mempooler, transaksjoner, adresser og mer.");
        }
      } else {
        this.activeTab = 3;
        this.seoService.setTitle("Electrum RPC");
        this.seoService.setDescription("Dokumentasjon for v\xE5rt Electrum RPC-grensesnitt: f\xE5 umiddelbar, praktisk og p\xE5litelig tilgang til en Esplora-instans.");
      }
    }
    ngOnDestroy() {
