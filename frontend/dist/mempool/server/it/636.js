    ...
    static #_2 = this.ɵcmp = /*@__PURE__*/_angular_core__WEBPACK_IMPORTED_MODULE_9__["ɵɵdefineComponent"]({
      ...
      consts: () => {
        ...
        if (typeof ngI18nClosureMode !== "undefined" && ngI18nClosureMode) {
          ...
          i18n_0 = "" + "[\uFFFD#1\uFFFD|\uFFFD#3\uFFFD]" + "" + "\uFFFD#2\uFFFD" + "mempool.space fornisce semplicemente dati sulla rete Bitcoin." + "\uFFFD/#2\uFFFD" + " Non pu\xF2 aiutarti con il recupero di fondi, problemi con il wallet, ecc." + "[\uFFFD/#1\uFFFD|\uFFFD/#3\uFFFD]" + "" + "[\uFFFD#1\uFFFD|\uFFFD#3\uFFFD]" + "Per qualsiasi richiesta di questo tipo, devi contattare l'entit\xE0 che ha contribuito a effettuare la transazione (software del wallet, exchange, ecc.)." + "[\uFFFD/#1\uFFFD|\uFFFD/#3\uFFFD]" + "";
        }
  ...
  class DocsComponent {
    ...
    ngDoCheck() {
      ...
      if (url[0].path === "faq") {
        ...
        this.seoService.setDescription("Ottieni risposte a domande comuni come: Cos'\xE8 una mempool? Perch\xE9 la mia transazione non viene confermata? Come posso eseguire la mia istanza del progetto Mempool Open Source? E altro ancora.");
        this.ogService.setManualOgImage('faq.jpg');
      } else if (url[1].path === "rest") {
        this.activeTab = 1;
        this.seoService.setTitle("REST API");
        if (this.stateService.network === 'liquid' || this.stateService.network === 'liquidtestnet') {
          this.seoService.setDescription("Documentazione per il servizio API REST liquid.network: ottieni informazioni su indirizzi, transazioni, risorse, blocchi e altro.");
        } else {
          this.seoService.setDescription("Documentazione per il servizio API REST mempool.space: ottieni informazioni su indirizzi, transazioni, blocchi, tariffe, mining, rete Lightning e altro ancora.");
        }
      } else if (url[1].path === "websocket") {
        this.activeTab = 2;
        this.seoService.setTitle("WebSocket API");
        if (this.stateService.network === 'liquid' || this.stateService.network === 'liquidtestnet') {
          this.seoService.setDescription("Documentazione per il servizio API WebSocket di liquid.network: ottieni informazioni in tempo reale su blocchi, mempool, transazioni, indirizzi e altro ancora.");
        } else {
          this.seoService.setDescription("Documentazione per il servizio API WebSocket mempool.space: ottieni informazioni in tempo reale su blocchi, mempool, transazioni, indirizzi e altro ancora.");
        }
      } else {
        this.activeTab = 3;
        this.seoService.setTitle("Electrum RPC");
        this.seoService.setDescription("Documentazione per la nostra interfaccia Electrum RPC: ottieni un accesso immediato, conveniente e affidabile a un'istanza di Esplora.");
      }
    }
    ngOnDestroy() {
