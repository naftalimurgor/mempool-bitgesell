    ...
    static #_2 = this.ɵcmp = /*@__PURE__*/_angular_core__WEBPACK_IMPORTED_MODULE_9__["ɵɵdefineComponent"]({
      ...
      consts: () => {
        ...
        if (typeof ngI18nClosureMode !== "undefined" && ngI18nClosureMode) {
          ...
          i18n_0 = "" + "[\uFFFD#1\uFFFD|\uFFFD#3\uFFFD]" + "" + "\uFFFD#2\uFFFD" + "mempool.space levert alleen gegevens over het Bitcoin-netwerk." + "\uFFFD/#2\uFFFD" + " Het kan u niet helpen met het ophalen van geld, portemonneeproblemen, enz." + "[\uFFFD/#1\uFFFD|\uFFFD/#3\uFFFD]" + "" + "[\uFFFD#1\uFFFD|\uFFFD#3\uFFFD]" + "Voor dergelijke verzoeken moet u contact opnemen met met de entiteit die heeft geholpen bij het uitvoeren van de transactie (portemonneesoftware, wisselkantoor, enz.)." + "[\uFFFD/#1\uFFFD|\uFFFD/#3\uFFFD]" + "";
        }
  ...
  class DocsComponent {
    ...
    ngDoCheck() {
      ...
      if (url[0].path === "faq") {
        ...
        this.seoService.setDescription("Krijg antwoorden op veelgestelde vragen zoals: Wat is een mempool? Waarom wordt mijn transactie niet bevestigd? Hoe kan ik mijn eigen exemplaar van The Mempool Open Source Project draaien? En meer.");
        this.ogService.setManualOgImage('faq.jpg');
      } else if (url[1].path === "rest") {
        this.activeTab = 1;
        this.seoService.setTitle("REST-API");
        if (this.stateService.network === 'liquid' || this.stateService.network === 'liquidtestnet') {
          this.seoService.setDescription("Documentatie voor de liquid.network REST API-service: krijg informatie over adressen, transacties, activa, blokken en meer.");
        } else {
          this.seoService.setDescription("Documentatie voor de mempool.space REST API-service: krijg informatie over adressen, transacties, blokken, kosten, mining, het Lightning-netwerk en meer.");
        }
      } else if (url[1].path === "websocket") {
        this.activeTab = 2;
        this.seoService.setTitle("WebSocket-API");
        if (this.stateService.network === 'liquid' || this.stateService.network === 'liquidtestnet') {
          this.seoService.setDescription("Documentatie voor de liquid.network WebSocket API-service: ontvang realtime informatie over blokken, mempools, transacties, adressen en meer.");
        } else {
          this.seoService.setDescription("Documentatie voor de mempool.space WebSocket API-service: ontvang realtime informatie over blokken, mempools, transacties, adressen en meer.");
        }
      } else {
        this.activeTab = 3;
        this.seoService.setTitle("Electrum-RPC");
        this.seoService.setDescription("Documentatie voor onze Electrum RPC-interface: krijg direct, gemakkelijk en betrouwbaar toegang tot een Esplora-instantie.");
      }
    }
    ngOnDestroy() {
