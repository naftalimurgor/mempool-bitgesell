    ...
    static #_2 = this.ɵcmp = /*@__PURE__*/_angular_core__WEBPACK_IMPORTED_MODULE_9__["ɵɵdefineComponent"]({
      ...
      consts: () => {
        ...
        if (typeof ngI18nClosureMode !== "undefined" && ngI18nClosureMode) {
          ...
          i18n_0 = "" + "[\uFFFD#1\uFFFD|\uFFFD#3\uFFFD]" + "" + "\uFFFD#2\uFFFD" + "mempool.space fournit simplement des donn\xE9es sur le r\xE9seau Bitcoin." + "\uFFFD/#2\uFFFD" + " Nous ne pouvons pas vous aider \xE0 r\xE9cup\xE9rer des fonds, \xE0 r\xE9soudre des probl\xE8mes de portefeuille, etc." + "[\uFFFD/#1\uFFFD|\uFFFD/#3\uFFFD]" + "" + "[\uFFFD#1\uFFFD|\uFFFD#3\uFFFD]" + "Pour toute demande de ce type, vous devez contacter l'entit\xE9 qui a aid\xE9 \xE0 effectuer la transaction (logiciel de portefeuille, \xE9change, etc.)." + "[\uFFFD/#1\uFFFD|\uFFFD/#3\uFFFD]" + "";
        }
  ...
  class DocsComponent {
    ...
    ngDoCheck() {
      ...
      if (url[0].path === "faq") {
        ...
        this.seoService.setDescription("Obtenez des r\xE9ponses aux questions courantes telles que : Qu\u2019est-ce qu\u2019une mempool ? Pourquoi ma transaction n'est-elle pas confirm\xE9e\xA0? Comment puis-je utiliser ma propre instance du projet Open Source Mempool\xA0? Et plus.");
        this.ogService.setManualOgImage('faq.jpg');
      } else if (url[1].path === "rest") {
        this.activeTab = 1;
        this.seoService.setTitle("API REST");
        if (this.stateService.network === 'liquid' || this.stateService.network === 'liquidtestnet') {
          this.seoService.setDescription("Documentation pour le service API REST liquid.network\xA0: obtenez des informations sur les adresses, les transactions, les actifs, les blocs, etc.");
        } else {
          this.seoService.setDescription("Documentation pour le service API REST mempool.space\xA0: obtenez des informations sur les adresses, les transactions, les blocs, les frais, le minage, le r\xE9seau Lightning, etc.");
        }
      } else if (url[1].path === "websocket") {
        this.activeTab = 2;
        this.seoService.setTitle("API WebSocket");
        if (this.stateService.network === 'liquid' || this.stateService.network === 'liquidtestnet') {
          this.seoService.setDescription("Documentation pour le service API liquid.network WebSocket\xA0: obtenez des informations en temps r\xE9el sur les blocs, les pools de m\xE9moire, les transactions, les adresses, etc.");
        } else {
          this.seoService.setDescription("Documentation pour le service API WebSocket mempool.space\xA0: obtenez des informations en temps r\xE9el sur les blocs, les pools de m\xE9moire, les transactions, les adresses, etc.");
        }
      } else {
        this.activeTab = 3;
        this.seoService.setTitle("\xC9lectrum RPC");
        this.seoService.setDescription("Documentation pour notre interface Electrum RPC : obtenez un acc\xE8s instantan\xE9, pratique et fiable \xE0 une instance Esplora.");
      }
    }
    ngOnDestroy() {
