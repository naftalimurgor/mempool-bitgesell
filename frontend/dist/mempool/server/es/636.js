    ...
    static #_2 = this.ɵcmp = /*@__PURE__*/_angular_core__WEBPACK_IMPORTED_MODULE_9__["ɵɵdefineComponent"]({
      ...
      consts: () => {
        ...
        if (typeof ngI18nClosureMode !== "undefined" && ngI18nClosureMode) {
          ...
          i18n_0 = "" + "[\uFFFD#1\uFFFD|\uFFFD#3\uFFFD]" + "" + "\uFFFD#2\uFFFD" + "mempool.space tan solo proporciona informaci\xF3n acerca de la red Bitcoin." + "\uFFFD/#2\uFFFD" + " No puede ayudarte a recuperar fondos, asundos con carteras, etc." + "[\uFFFD/#1\uFFFD|\uFFFD/#3\uFFFD]" + "" + "[\uFFFD#1\uFFFD|\uFFFD#3\uFFFD]" + "Para estas solicitudes, necesitas ponerte en contacto con la entidad que te ayud\xF3 a hacer la transacci\xF3n (software del monedero, casa de cambio, etc)." + "[\uFFFD/#1\uFFFD|\uFFFD/#3\uFFFD]" + "";
        }
...
let DocsComponent = /*#__PURE__*/(() => {
  class DocsComponent {
    ...
    ngDoCheck() {
      ...
      if (url[0].path === "faq") {
        this.activeTab = 0;
        this.seoService.setTitle("Preguntas frequentes");
        this.seoService.setDescription("Obt\xE9n respuestas a preguntas comunes como: Qu\xE9 es una mempool? Por qu\xE9 no se confirma mi transacci\xF3n? C\xF3mo puedo correr mi instancia del Proyecto Open Source Mempool? Y m\xE1s.");
        this.ogService.setManualOgImage('faq.jpg');
      } else if (url[1].path === "rest") {
        this.activeTab = 1;
        this.seoService.setTitle("REST API");
        if (this.stateService.network === 'liquid' || this.stateService.network === 'liquidtestnet') {
          this.seoService.setDescription("Documentaci\xF3n para el servicio REST API de liquid.network: obt\xE9n informaci\xF3n sobre direcciones, activos, bloques y m\xE1s.");
        } else {
          this.seoService.setDescription("Documentaci\xF3n para el servicio REST API: obt\xE9n informaci\xF3n sobre direcciones, transacciones, bloques, tasas, la red Lightning y m\xE1s.");
        }
      } else if (url[1].path === "websocket") {
        this.activeTab = 2;
        this.seoService.setTitle("WebSocket API");
        if (this.stateService.network === 'liquid' || this.stateService.network === 'liquidtestnet') {
          this.seoService.setDescription("Documentaci\xF3n para el servicio Websocket API de liquid.network: obt\xE9n informaci\xF3n en tiempo real sobre bloques, mempools, transacciones, direcciones y m\xE1s.");
        } else {
          this.seoService.setDescription("Documentaci\xF3n para el servicio Websocket API de mempool.space: obt\xE9n informaci\xF3n en tiempo real sobre bloques, mempools, transacciones, direcciones y m\xE1s.");
        }
      } else {
        this.activeTab = 3;
        this.seoService.setTitle("Electrum RPC");
        this.seoService.setDescription("Documentaci\xF3n para la interfaz Electrum RPC: obt\xE9n acceso instant\xE1neo, c\xF3modo y fiable a una instancia Esplora.");
      }
    }
    ngOnDestroy() {
